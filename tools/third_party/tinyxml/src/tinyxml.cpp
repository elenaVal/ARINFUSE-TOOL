...
const TiXmlNode* TiXmlNode::FirstChild(const char * _value, bool anyCasing /*=false*/) const
{
	...
	for ( node = firstChild; node; node = node->next )
	{
		if (anyCasing)
		{
			std::string str_value = _value;
			std::string str_node_value = node->SValue().c_str();
			std::transform(str_value.begin(), str_value.end(), str_value.begin(), ::toupper);
			std::transform(str_node_value.begin(), str_node_value.end(), str_node_value.begin(), ::toupper);
			if (str_value == str_node_value)
			{
				return node;
			}
		}
...
TiXmlNode* TiXmlNode::FirstChild(const char * _value, bool anyCasing /*=false*/)
{
	TiXmlNode* node;
	for ( node = firstChild; node; node = node->next )
	{
